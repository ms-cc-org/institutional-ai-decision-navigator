...
2. `config/institution.ts` supplies the public institution identity, logo paths, contact information, deployment mode, and explicit attribution choice without changing the core.
3. `lib/intents.ts` defines the typed, application-level intent paths, seed decision IDs, relevant domains, and context questions separately from the ontology.
4. `lib/applicability-contexts.ts` defines separate, controlled routing metadata for data and requirement contexts. It maps only to existing decision IDs and never changes ontology evidence profiles.
5. `lib/local-guidance.ts` validates optional institution-local policies and annotations from `data/local/institution-guidance.json`. These records map additively to existing decision IDs and carry `institution_local` provenance.
6. `components/IntentNavigator.tsx` presents three primary entry modes. The guided mode uses `components/ProfileForm.tsx`; the experimental situation mode uses `lib/situation-interpreter.ts`; the explorer filters the same canonical decisions.
7. The current intent, answers, diagnostic state, situation interpretation, and optional profile are stored only in browser local storage using versioned structures where schemas have evolved.
8. `lib/intent-engine.ts` activates deterministic answer branches, traverses ontology prerequisites, and emits no more than five primary recommendations plus a few next or conditional decisions.
9. `components/FocusedRoadmap.tsx` renders plain-language explanations and actions first. Evidence IDs and dependency links sit in expandable secondary sections.
10. Detail routes render user-facing ontology content first, MS-CC core evidence second, optional institution-local guidance in a separate section, and technical metadata and working-group validation through progressive disclosure.
...
Institution configuration and local guidance are intentionally outside `data/ontology.json`. Local records may annotate existing decisions but cannot silently replace them or change evidence scores. The footer exposes core ontology, application, and local-config versions; upstream merging and any future decision-override/versioning model remain manual design questions.
