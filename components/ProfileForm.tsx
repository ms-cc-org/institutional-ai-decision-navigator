...
export function ProfileForm() {
  ...
  <fieldset className="field wide"><legend><b>08</b>Primary AI objectives <small>Select one or more</small></legend><div className="checks">{objectives.map(([value,label])=><label key={value}><input type="checkbox" checked={profile.primaryObjectives.includes(value)} onChange={(e)=>setProfile({...profile,primaryObjectives:e.target.checked?[...profile.primaryObjectives,value]:profile.primaryObjectives.filter((x)=>x!==value)})}/><span>{label}</span></label>)}</div>{profile.primaryObjectives.length===0&&<p className="field-error" role="alert">Select at least one objective.</p>}</fieldset></div><div className="submit-row"><p>Results are deterministic and traceable to ontology data.</p><button type="submit">Build my roadmap <span>→</span></button></div></form>;
}
